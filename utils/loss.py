...
class YOLOLoss(nn.Module):
    ...
    def forward(self, l, input,imgs = None, targets=None):
        ...
        #   targets代表的是真实框,格式为xywh，class，xywh都是代表在整张图片中的占比
        #----------------------------------------------------#
        #--------------------------------#
        #   获得图片数量，特征层的高和宽
        #   13和13
        ...
        #   如果特征层为13x13的话，一个特征点就对应原来的图片上的32*32个像素点
        #   如果特征层为26x26的话，一个特征点就对应原来的图片上的16*16个像素点
        #   如果特征层为52x52的话，一个特征点就对应原来的图片上的8*8个像素点
        ...
        #-------------------------------------------------#
        scaled_anchors  = [(a_w / stride_w, a_h / stride_h) for a_w, a_h in self.anchors]#anchor在特征层面的大小
        #-----------------------------------------------#
